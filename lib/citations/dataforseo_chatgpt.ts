...
// DataForSEO ChatGPT LLM Scraper — scrapes the actual ChatGPT web interface.
// Returns what users really see: AI response + exact web sources cited.
...
export class DataForSEOChatGPTConnector implements PlatformConnector {
  readonly platform = "openai" as const;

  async run(promptText: string, trackedDomain: string): Promise<CitationResult> {
    const auth = dfsAuth();

    const res = await fetch("https://api.dataforseo.com/v3/ai_optimization/chat_gpt/llm_scraper/live/advanced/", {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      ...
      throw new Error(`DataForSEO ChatGPT error ${res.status}: ${err.slice(0, 300)}`);
    }
    ...
    // Use search_results URLs for reliable citation detection
    const urls = searchResults.map(r => r.url ?? "").filter(Boolean);
    // Also extract inline markdown URLs as fallback
    const mdUrls = [...markdown.matchAll(/\]\((https?:\/\/[^)]+)\)/g)].map(m => m[1] ?? "");
    const allUrls = [...new Set([...urls, ...mdUrls])];

    // Strip markdown links for clean mention detection in text
