...
export async function GET(req: NextRequest) {
  ...
  const empty = { prompts: [], runs: [], runDetails: [], citationShare: {}, shareOfVoice: null, topCompetitors: [], topPages: [], trend: [], meta: { days, since, totalRuns: 0 } };
  ...
  const { data: runs } = await sb
    .from("citation_runs")
    .select("id, prompt_id, platform, run_at, cited, citation_position, cited_url, competitor_domains, raw_response")
    .in("prompt_id", promptIds)
    .gte("run_at", since)
    .order("run_at", { ascending: false });

  if (!runs) return NextResponse.json({ ...empty, prompts });
  ...
  for (const run of runs) {
    ...
    if (run.cited) {
      const ourDomain = prompts.find(p => p.id === run.prompt_id)?.tracked_url ?? "our_domain";
      domainMentions[ourDomain] = (domainMentions[ourDomain] ?? 0) + 1;
    }
  }
  ...
  for (const run of runs) {
    for (const d of (run.competitor_domains as string[] | null) ?? []) {
  ...
  const trend = Object.entries(trendByWeek)
    ...
    .map(([week, { cited, total }]) => ({ week, share: total > 0 ? Math.round((cited / total) * 100) : 0, cited, total }));

  // ── Run details for the analysis table ──────────────────────────────────
  // Keep only the latest run per (prompt_id × platform) to avoid duplicates in the table
  ...
  const runDetails = Object.values(latestByKey).map(run => ({
    ...
    responseExcerpt: extractExcerpt(run.platform, run.raw_response),
