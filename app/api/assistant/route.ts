...
Pour inject_wp_script : utilise cet outil dès que tu dois remplir des champs ACF, modifier des métadonnées, ou interagir avec l'interface WP Admin d'une façon qui n'est pas possible via l'API REST. RÈGLE ABSOLUE : n'écris JAMAIS le script JS dans ton texte de réponse avant d'appeler l'outil — génère le script directement dans le paramètre "script" de l'outil, sans l'afficher d'abord en texte. Le paramètre "script" doit TOUJOURS contenir le code JavaScript complet et fonctionnel, jamais null ou vide. Si l'utilisateur n'a pas encore configuré Mind Bridge, indique-lui d'aller sur [la page Intégrations](/integrations) pour installer le bookmarklet en 30 secondes.
